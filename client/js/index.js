  ...
  playRound (players) {
    players.forEach(player => {
      ...
      if (!answer) {
        ...
        this.movies.pop(this.movies.indexOf(answer))
      }
    })
    return players;
  }
...
class Player {
  ...
  guess () {
    return prompt('Whats your guess?');
  }

  answer (title, answer) {
    const answerHTML = `
      <li class="card game__guess">
        <p class='game__answer ${answer ? 'correct-answer' : 'incorrect-answer'}'>${title}</p>
  ...
  answer (title) {
    const answerHTML = `
      <li class="card game__guess game__guess-doug">
        <p class='game__answer correct-answer'>${title}</p>
